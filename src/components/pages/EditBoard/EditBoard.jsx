...
function EditBoard({ board }) {
  ...
  function onToggle(name, status) {
    ...
    } else if (name === "multiple_at_a_time") {
      if (settings.multipleAtATime) {
        settings.multipleAtATime = false;
      } else if (!settings.multipleAtATime) {
        settings.multipleAtATime = true;
      }
    }
  }
  //#endregion
      ...
      <div className="edit__bottom">
        <div className="edit__edit_settings_wrapper">
          ...
          <div className="edit__setting">
            <p className="edit__setting_label">
              ...
              <span
                className="edit__setting__tooltip"
                data-tip="If you enable this it will override the audio player setting making it only play one at a time."
              >
                &#x1F6C8;
              </span>
            </p>
            <Toggle
              name="multiple_at_a_time"
              onToggle={onToggle}
              defaultValue={
                settings.multipleAtATime ? settings.multipleAtATime : false
              }
            />
          </div>
          <div className="edit__setting">
        ...
        <div className="edit__edit_sounds_wrapper">
          <div className="edit__edit_sounds_top">
            ...
            <div className="edit__right">
              <Btn content="New Sound" onClick={() => setShowNew(true)} />
            </div>
          </div>
          {/* <Table
						edit={true}
          ...
          <ReactTabulator
            ref={(r) => {
              setRows !== undefined && r !== null ? setRows(r) : (r = null);
            }}
            data={sounds}
            columns={columns}
            layout={"fitColumns"}
